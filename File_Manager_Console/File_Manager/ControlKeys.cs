...
namespace TotalComanderOOP
{
    internal class ControlKeys
    {
        ...
        public UserCommandInfo Start()
        {
            ...
            do
            {
                ...
                switch (infoKey.Key)
                {
                    case ConsoleKey.UpArrow:
                        if(PressUpp())
                        {
                            return new UserCommandInfo(CommandKey.GetLength,
                    ...
                    //==============================================================================
                    case ConsoleKey.F5:
                        return new UserCommandInfo(CommandKey.Copy,
                           Path.Combine(DataDirs[DataDirectores.Select_Window].DirHome,
                                        DataDirs[DataDirectores.Select_Window].AllDirectoris[Select])
                    ...
                    case ConsoleKey.Tab:
                        PressTAB();
                        return new UserCommandInfo(CommandKey.Chenge_Windows);
                    //==============================================================================
                    case ConsoleKey.Escape:
                        return new UserCommandInfo(CommandKey.Exit);
                    //==============================================================================
                    case ConsoleKey.Enter:
                        ...
                        if (command.Length > 0)
                        {
                            ...
                            return new UserCommandInfo(CommandKey.LineCommand, command.ToString());
                        }
                        else
                        {
                            if (Select == 0)
                            {
                                return new UserCommandInfo(CommandKey.ChangeDirUpp);
                            }
                            if (DataDirs[DataDirectores.Select_Window].Dirs_Info.Length > 0)
                            {
                                int select2 = Select;
                                ChangeSelect(0);
                                return new UserCommandInfo(CommandKey.ChangeDir,
            ...
            while (status);
            return new UserCommandInfo(CommandKey.Null);
        }
